...
for count=1:1:maxgen  
   for i=1:1:NP
       ...
       %disp(trial);
       if (count==1 && i==1)
           trial=load('ResultPshortmodel1bLS0115b.mat');
           trial=cell2mat(struct2cell(trial)); %zeros(1,D);
       end
       score=model1bErrorFls(tspan,IC,trial); %go to use fitness funtion to evaluate the error
...
save('ResultPshortmodel1bLS0116.mat','trial0');
