...
for count=1:1:maxgen  
   for i=1:1:NP
       ...
       %disp(trial);
       if (count==1 && i==1)
           trial=load('ResultPshortmodel2b0321a.mat');
           trial=cell2mat(struct2cell(trial)); %zeros(1,D);
       end
       score=model2bErrorFabs(tspan,IC,trial); %go to use fitness funtion to evaluate the error
...
save('ResultPshortmodel2b0425.mat','trial0');
