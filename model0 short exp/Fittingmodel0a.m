...
tspan=[0,t1,t2,t3,t4]; %,build time range t5,t6,t7,t8
...
for count=1:1:maxgen  
   for i=1:1:NP
       ...
       b=floor(rand*(NP-2))+2;
       c=floor(rand*(NP-3))+3;
       j=floor(rand*D)+1;       %pick up a random in D
       ...
       if (count==1 && i==1) % && = and;
           trial=load('ResultPshortmodel0a0705a.mat');
           trial=cell2mat(struct2cell(trial)); %zeros(1,D);
       end
       %disp(trial);
       trial(5)=0; trial(10)=0; %K_m^*=K_n^*=0;
       score=model0aErrorFls(tspan,IC,trial); %go to use fitness funtion to evaluate the error
...
save('ResultPshortmodel0a0705b.mat','trial0');
