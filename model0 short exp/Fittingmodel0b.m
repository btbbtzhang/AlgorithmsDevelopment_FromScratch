...
for count=1:1:maxgen  
   for i=1:1:NP
       ...
       if (count==1 && i==1)
           trial=load('ResultPshortmodel0b0404.mat');
           trial=cell2mat(struct2cell(trial)); %zeros(1,D);
       end
       %disp(trial);
       score=model0bErrorFabs(tspan,IC,trial); %go to use fitness funtion to evaluate the error
...
save('ResultPshortmodel0b0425.mat','trial0');
